      ...
      {showGen && (
        <div className="mb-6 p-4 rounded-lg" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
          ...
          <div className="grid grid-cols-5 gap-3 mb-3">
            <div>
              <label className="block text-xs mb-1" style={{ color: "var(--text-muted)", fontFamily: '"JetBrains Mono", monospace' }}>类型</label>
            ...
            <div>
              <label className="block text-xs mb-1" style={{ color: "var(--text-muted)", fontFamily: '"JetBrains Mono", monospace' }}>数值</label>
              <input type="number" value={genValue} onChange={(e) => setGenValue(e.target.value)} min="1" className="w-full px-2 py-1.5 rounded-md text-xs outline-none" style={{ background: "var(--bg)", border: "1px solid var(--border)", color: "var(--text)", fontFamily: '"JetBrains Mono", monospace' }} />
            </div>
            <div>
              <label className="block text-xs mb-1" style={{ color: "var(--text-muted)", fontFamily: '"JetBrains Mono", monospace' }}>数量</label>
              <input type="number" value={genCount} onChange={(e) => setGenCount(e.target.value)} min="1" max="500" className="w-full px-2 py-1.5 rounded-md text-xs outline-none" style={{ background: "var(--bg)", border: "1px solid var(--border)", color: "var(--text)", fontFamily: '"JetBrains Mono", monospace' }} />
            </div>
            <div>
              <label className="block text-xs mb-1" style={{ color: "var(--text-muted)", fontFamily: '"JetBrains Mono", monospace' }}>有效天数</label>
              <input type="number" value={genExpireDays} onChange={(e) => setGenExpireDays(e.target.value)} min="1" className="w-full px-2 py-1.5 rounded-md text-xs outline-none" style={{ background: "var(--bg)", border: "1px solid var(--border)", color: "var(--text)", fontFamily: '"JetBrains Mono", monospace' }} />
            </div>
            <div>
              <label className="block text-xs mb-1" style={{ color: "var(--text-muted)", fontFamily: '"JetBrains Mono", monospace' }}>批次号(可选)</label>
              <input type="text" value={genBatchNo} onChange={(e) => setGenBatchNo(e.target.value)} placeholder="自动生成" className="w-full px-2 py-1.5 rounded-md text-xs outline-none" style={{ background: "var(--bg)", border: "1px solid var(--border)", color: "var(--text)", fontFamily: '"JetBrains Mono", monospace' }} />
            </div>
          </div>
          <button type="button" onClick={handleGenerate} disabled={isPending} className="px-4 py-1.5 text-xs rounded-md" style={{ background: "var(--accent)", color: "var(--bg)", fontFamily: '"JetBrains Mono", monospace', opacity: isPending ? 0.7 : 1 }}>
            {isPending ? "生成中..." : "生成"}
          </button>
          ...
          {genResult && (
            <div className="mt-3 p-3 rounded-lg" style={{ background: "var(--teal-dim)", border: "1px solid var(--teal)" }}>
              <div className="flex items-center justify-between mb-2">
                ...
                <button type="button" onClick={() => copyToClipboard(genResult.cards.map((c: any) => `${c.cardNo},${c.cardSecret}`).join("\n"))} className="inline-flex items-center gap-1 text-[10px]" style={{ color: "var(--teal)", fontFamily: '"JetBrains Mono", monospace' }}>
                  <Copy size={11} aria-hidden="true" />
                  {copied ? "已复制" : "复制全部"}
                </button>
              </div>
              <div className="max-h-40 overflow-y-auto text-[10px]" style={{ fontFamily: '"JetBrains Mono", monospace', color: "var(--text-secondary)" }}>
      ...
      <div className="flex gap-3 mb-4">
        <div className="flex gap-1">
          {[{ id: "", label: "全部类型" }, { id: "VIP_DAYS", label: "VIP" }, { id: "PAID_ARTICLE", label: "付费文章" }, { id: "BALANCE", label: "余额" }].map((f) => (
            <button key={f.id} type="button" onClick={() => { setTypeFilter(f.id); setPage(1); }} className="px-2 py-1 rounded-md text-[10px] border transition-colors" style={{ borderColor: typeFilter === f.id ? "var(--accent)" : "var(--border)", background: typeFilter === f.id ? "var(--accent-dim)" : "transparent", color: typeFilter === f.id ? "var(--accent)" : "var(--text-muted)", fontFamily: '"JetBrains Mono", monospace' }}>
              {f.label}
            </button>
          ))}
        </div>
        <div className="flex gap-1">
          {[{ id: "", label: "全部状态" }, { id: "ACTIVE", label: "可用" }, { id: "USED", label: "已使用" }, { id: "DISABLED", label: "已禁用" }].map((f) => (
            <button key={f.id} type="button" onClick={() => { setStatusFilter(f.id); setPage(1); }} className="px-2 py-1 rounded-md text-[10px] border transition-colors" style={{ borderColor: statusFilter === f.id ? "var(--accent)" : "var(--border)", background: statusFilter === f.id ? "var(--accent-dim)" : "transparent", color: statusFilter === f.id ? "var(--accent)" : "var(--text-muted)", fontFamily: '"JetBrains Mono", monospace' }}>
              {f.label}
            </button>
          ))}
        </div>
            ...
            {loading ? (
              ...
              <tr key={c.id} style={{ borderTop: "1px solid var(--border)" }}>
                ...
                <td className="px-3 py-2">
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ background: `${STATUS_COLORS[c.status]}15`, color: STATUS_COLORS[c.status], fontFamily: '"JetBrains Mono", monospace' }}>
                    {STATUS_LABELS[c.status]}
                  </span>
                </td>
                <td className="px-3 py-2 text-[10px]" style={{ color: "var(--text-muted)", fontFamily: '"JetBrains Mono", monospace' }}>{c.batchNo || "—"}</td>
